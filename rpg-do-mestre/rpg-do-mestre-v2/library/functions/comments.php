...
function framework_discussion_title( $type = NULL, $echo = true ) {
	...
	switch( $type ) {
		case 'comment' :
			$count = $comment_count;
			$many  = apply_filters( 'framework_many_comments', (string) '% Coment&aacute;rios' ); // Available filter: framework_many_comments
			$none  = apply_filters( 'framework_no_comments', (string) '0 Coment&aacute;rios' ); // Available filter: framework_no_comments
			$one   = apply_filters( 'framework_one_comment', (string) '1 Coment&aacute;rio' ); // Available filter: framework_one_comment
			break;
		case 'pings' :
			$count = $ping_count;
	...
	if ( $number ) {
		...
		$discussion_title .= '<span class="'. $type .'-title-meta">' . $number . ' para ' . '</span>';
 ...
 * @todo needs filter
 */
function framework_comment_meta( $meta_format = '%date% às %time% | %link% %edit%' ) {	
...
function framework_comment_navigation() {
	...
	if ( $num > get_option( 'comments_per_page' ) ) {		
		$paged_links = paginate_comments_links( array(
			'type' => 'array',
			'echo' => false,
			'prev_text' => '&laquo; P&aacute;gina anterior',
			'next_text' => 'Pr&oacute;xima p&aacute;gina &raquo;' ) );
...
function framework_pings_callback( $comment, $args, $depth ) {
	...
	$byline = apply_filters( 'framework_pings_callback_byline', (string) 'Por ' ); // Available filter: framework_pings_callback_byline
	$time = apply_filters( 'framework_pings_callback_time', (string) ' em ' ); // Available filter: framework_pings_callback_time
	$when = apply_filters( 'framework_pings_callback_when', (string) ' em ' ); // Available filter: framework_pings_callback_time
	?>
