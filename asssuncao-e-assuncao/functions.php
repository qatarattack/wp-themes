...
/* Main Nav Menu */
register_nav_menu( 'header_menu','Menu de navegação para o cabeçalho' );
	...
	register_post_type( 'portfolio', /* this can be seen at the URL as a parameter and a unique id for the custom post */
		array(
			'labels' => array(
				...
				'singular_name' => __( "Portfolio", 'textdomain_finale' ) /* The Label of the custom post */
			),
			'public' => true,
			'has_archive' => true,
...
function portfolio_taxonomie() {
	...
	register_taxonomy(
		...
		array(
            ...
            'hierarchical' => true,
			'labels' => array(
				'name' => __( 'Cartegoria do Protfolio', 'textdomain_finale' ),
				'singular_name' => __( 'Cartegoria do Protfolio', 'textdomain_finale' )
			),
		)
	);
}
...
?>
		...
		<!--<a href="#" class="comment_name">adminstração</a>-->
        ...
        <?php comment_reply_link(array_merge( $args, array('add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth'], reply_text => 'Responder'))) ?>
		</div>

		<!-- Start Comment Text -->
        <div class="comment_content">
        <?php if ($comment->comment_approved == '0') : ?>
		<em class="comment-awaiting-moderation"><?php _e('Comentário esperando moderação.','textdomain_finale') ?></em>
        <br /><br />
        <?php endif; ?>
		<?php comment_text() ?>		
