...
<div id="wrapper">
	...
	<header id="header" class="col-full">
		...
		<nav class="woocommerce-nav">
			<ul>
				<li class="my-account"><a href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>" title="<?php _e('Minha conta','woothemes'); ?>"><?php _e('Conta','woothemes'); ?></a></li>
				<li class="cart">
					<?php _e('Carrinho:', 'woothemes'); ?> <a href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php _e('Veja seu carrinho', 'woothemes'); ?>"><?php echo sprintf(_n('%d item', '%d itens', $woocommerce->cart->get_cart_contents_count(), 'woothemes'), $woocommerce->cart->get_cart_contents_count());?></a>
				</li>
				<li><?php echo $woocommerce->cart->get_cart_total(); ?></li>
				<li class="checkout"><a href="<?php echo $woocommerce->cart->get_checkout_url()?>"><?php _e('Finalizar','woothemes') ?></a></li>
			</ul>
		</nav>
		...
		<div id="search-top">
	    	...
	    	<form role="search" method="get" id="searchform" class="searchform" action="<?php echo home_url(); ?>">
				<label class="screen-reader-text" for="s"><?php _e('Procurar por:', 'woothemes'); ?></label>
				<input type="text" value="<?php the_search_query(); ?>" name="s" id="s"  class="field s" placeholder="<?php _e('Buscar', 'woothemes'); ?>" />
				<input type="image" class="submit btn" name="submit" value="<?php _e('Buscar', 'woothemes'); ?>" src="<?php echo get_template_directory_uri(); ?>/images/ico-search-grad.png" alt="Search">
	...
	<nav id="navigation" class="col-full" role="navigation">
        ...
        <ul id="main-nav" class="nav fl">
        	...
        	if ( isset($woo_options[ 'woo_custom_nav_menu' ]) AND $woo_options[ 'woo_custom_nav_menu' ] == 'true' ) {
	            ...
	            <li class="<?php echo $highlight; ?>"><a href="<?php echo home_url( '/' ); ?>"><?php _e( 'Inicial', 'woothemes' ) ?></a></li>
	            <?php
